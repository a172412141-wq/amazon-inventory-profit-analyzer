...
页面按“上传与校验 → 经营总览 → SKU 角色 → 父体/SPU/品线诊断 → 数据质量 → 导出报告”组织。总览默认优先展示 7 天数据，并可用快速周期按钮切换 7 天/14 天；详细指标保留在可展开区域。
...
- `spu`
- `product_line`
- `category_level_3`（三级分类；用于侧边栏联动筛选）
- `size`（用于同尺寸横向比较；没有该字段时报告会标记低置信度）
- `category_level_1`
- `sales_7d_amount`
- `sales_14d_amount`
- `ad_impressions`
...
- `cvr`
- `ad_cvr`
- `acoas`（系统会按 `ad_spend / 当前周期总销售额` 自动计算，默认 7 天，可切换 14 天）
- `sessions_7d`
- `sessions_14d`
...
- `ACOAS` -> `acoas`
- `尺寸` / `规格` -> `size`
- `三级分类` / `三级品类` / `三级类目` -> `category_level_3`
- `14天会话数` -> `sessions_14d`
- `广告花费` -> `ad_spend`
- `广告销售额` -> `ad_sales`
...
4. 毛利润为负时暂缓补货或清货。
5. 广告无转化、ACOS 超过毛利率时控广告；ACOAS 按筛选后同周期的 `广告花费 / 总销售额` 展示整体广告花费占销售额比例。
...
总览 Dashboard 会展示 CPC、CTR、CVR、广告CVR、ACOS、ACOAS、广告订单占比、可售库存天数、在途库存天数、库龄超过90天合计数量，以及 61-90 天、91-180 天、180 天以上可售库存量。广告订单占比公式为 `广告订单 / 总订单`，总订单优先读取原表字段，没有时使用 `14天订单商品总数`。ACOAS 公式为 `广告花费 / 当前周期总销售额`，默认 7 天，可切换 14 天，并始终在当前筛选范围内重新汇总。其中 CPC/CTR/CVR 优先使用原表字段；如果原表缺失，会按广告点击、曝光、会话、广告订单等字段自动计算。

`SPU / 品线分析` 页面包含基于《Fang经营关系诊断模型》的单品线经营诊断。每次只能选择一条品线，报告层按“数据可信度 -> 经营关系错配 -> 原因分级 -> SKU角色与资源配置 -> 行动闭环”输出结果，并遵循 `周转 > 规模 > 毛利润 > 毛利率` 的默认经营优先序。

Fang 报告会先检查广告样本规模。广告花费或点击未达到 `thresholds.yaml` 中的暂定可靠门槛时，高 ACOAS 只作为待验证方向，不会被定义为品线核心矛盾；该降级不改变系统已经生成的 `final_action` 和 `priority`。

品线 ToDo List 包含对象、问题、证据、原因等级、经营影响、动作、负责人、开始/完成时间、观察周期、成功标准和失败预案。页面允许编辑 ToDo 状态、负责人和时间。报告层只读取已有分析结果，不修改原始数据、`sku_role`、`final_action` 或系统 `priority`；系统动作和系统优先级会作为独立字段保留。
