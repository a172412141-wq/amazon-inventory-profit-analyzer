...
`SPU / 品线分析` 页面包含单品线经营诊断。每次只能选择一条品线，报告层会按“品线 -> 父ASIN -> SKU -> SKU经营角色”的顺序输出经营结论、核心指标、SKU角色结构、重点问题SKU、重点机会SKU和行动清单。该报告只读取已有分析结果，不修改原始数据、`sku_role`、`final_action` 或系统 `priority`。
