...
角色判断基于 `parent_asin` 父体维度的相对表现。系统会计算父体内销量占比、销售额占比、广告花费占比、毛利润占比和可售库存占比，并输出：

- `sku_role_candidates`：候选标签，允许一个 SKU 同时具备多个候选特征。
- `sku_role_reason`：中文解释，说明为什么归入该角色。

最终 `sku_role` 互斥，优先级大致为：

1. 严重库存、利润或广告异常：`低效异常 SKU`
2. 父体内销量/销售贡献靠前且利润为正：`主力 SKU`
3. 毛利润贡献或毛利率较好：`利润 SKU`
4. 广告、曝光、点击或销量贡献较高但利润效率一般：`引流 SKU`
...
- `sku_roles`：父体内高/中/低占比、库存占比和引流利润折扣阈值。
