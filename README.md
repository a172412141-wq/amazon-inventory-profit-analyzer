...
角色判断基于 `parent_asin` 父体维度的相对表现。系统会计算父体内广告花费占比、父体平均日均销量、父体平均毛利率，并输出：
...
2. `主力 SKU`：日均销量高于父体内所有子 SKU 的日均销量算术平均值，且毛利率高于父体内所有子 SKU 的毛利率算术平均值
3. `利润 SKU`：毛利率高于父体平均值 `50%` 以上
4. `低效异常 SKU`：不满足上述三类条件

父体平均值按每个 `parent_asin` 单独分组计算，使用子 SKU 指标值的算术平均，不使用合计值。例如同一父体下两个子 SKU 的日均销量为 4 和 6，则父体平均日均销量为 5。
