...
总览 Dashboard 会展示 CPC、CTR、CVR、广告CVR、广告订单占比、可售库存天数、在途库存天数，以及 61-90 天、91-180 天、180 天以上可售库存的 SKU 数和库存量。其中 CPC/CTR/CVR 优先使用原表字段；如果原表缺失，会按广告点击、曝光、会话、广告订单等字段自动计算。
