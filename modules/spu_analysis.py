...
def _status_and_recommendation(row: pd.Series, high_sales_threshold: float) -> tuple[str, str]:
    ...
    healthy_stock = not pd.isna(stock_days) and 30 <= stock_days <= 120
...
def analyze_spu(df: pd.DataFrame) -> pd.DataFrame:
